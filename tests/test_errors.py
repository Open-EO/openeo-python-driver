...
def get_defined_exceptions(mod=openeo_driver.errors) -> Dict[str, List[Type[OpenEOApiException]]]:
    ...
    defined_exceptions = [
        ...
        if isclass(value) and issubclass(value, OpenEOApiException)
    ]
...
def test_error_code(error_code):
    ...
    for exception_cls in exceptions:
        ...
        assert error_code.lower() in exception_cls.__name__.lower() or exception_cls is OpenEOApiException
        ...
        placeholders = spec_helper.extract_placeholders(error_spec["message"])
        default_init = exception_cls.__init__ is OpenEOApiException.__init__ and exception_cls is not OpenEOApiException
        if placeholders and default_init:
