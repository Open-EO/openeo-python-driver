...
from openeo_driver.users import User, user_id_b64_decode
...
def register_views_batch_jobs(
    ...
    def list_job_results(job_id, user: User):
        ...
        doc = backend_implementation.batch_jobs.list_job_results(
            job_id=job_id, user_id=user.user_id, partial=partial, api_version=requested_api_version()
        )
    ...
    def list_job_results_signed(job_id, user_base64, secure_key):
        ...
        with TimingLogger(f"_list_job_results({job_id=}, {user_id=}, {partial=})", _log):
            doc = backend_implementation.batch_jobs.list_job_results(
                job_id=job_id, user_id=user_id, partial=partial, api_version=requested_api_version()
            )
        return flask.jsonify(doc)
    ...
    def get_job_result_item_signed(job_id, user_base64, secure_key, item_id):
        ...
        doc = backend_implementation.batch_jobs.get_item_metadata_doc(
            job_id=job_id, user_id=user_id, item_id=item_id, format="openeo100"
        )
        resp = flask.jsonify(doc)
    ...
    def get_job_result_item11_signed(job_id, user_base64, secure_key, item_id):
        ...
        signer.verify_job_item(signature=secure_key, job_id=job_id, user_id=user_id, item_id=item_id, expires=expires)
        doc = backend_implementation.batch_jobs.get_item_metadata_doc(
            job_id=job_id, user_id=user_id, item_id=item_id, format="stac11"
        )
        resp = flask.jsonify(doc)
        resp.mimetype = STAC_ITEM_MEDIA_TYPE
        return resp
    ...
    def get_job_result_item(job_id: str, item_id: str, user: User) -> flask.Response:
        doc = backend_implementation.batch_jobs.get_item_metadata_doc(
            job_id=job_id, user_id=user.user_id, item_id=item_id, format="openeo100"
        )
        resp = flask.jsonify(doc)
        resp.mimetype = STAC_ITEM_MEDIA_TYPE
        return resp
    ...
    def get_job_result_item11(job_id: str, item_id: str, user: User) -> flask.Response:
        doc = backend_implementation.batch_jobs.get_item_metadata_doc(
            job_id=job_id, user_id=user.user_id, item_id=item_id, format="stac11"
        )
        resp = flask.jsonify(doc)
        resp.mimetype = STAC_ITEM_MEDIA_TYPE
        return resp
