...
@non_standard_process(
    ...
    .param(
        name="geometries",
        description="Input geometry to add buffer to, vector-cube or GeoJSON(deprecated).",
        schema=[
    ...
    .param(
        name="distance", description="The distance of the buffer in meters.", schema={"type": "number"}, required=True
    )
    .returns(
        description="Output geometry (GeoJSON object) with the added or subtracted buffer",
...
@non_standard_process(
    ProcessSpec(
        "get_geometries",
        description="Reads vector data from a file or a URL or get geometries from a FeatureCollection",
    )
...
@non_standard_process(
    ProcessSpec(
        id="to_vector_cube", description="[EXPERIMENTAL:] Converts given data (e.g. GeoJson object) to a vector cube."
    )
...
@non_standard_process(
    ...
    .param(
        "target",
        description="A raster data cube used as reference.",
