...
from openeo_driver.datastructs import StacAsset, StacItem
...
class SaveResult:
    ...
    def write_assets(self, directory: Union[str, Path]) -> Dict[str, StacItem]:
        raise NotImplementedError

    @classmethod
    def wrap_assets_in_item(cls, assets:Dict[str,StacAsset]) -> dict:
...
class JSONResult(SaveResult):
    ...
    def write_assets(self, directory: Union[str, Path]) -> Dict[str, StacItem]:
        """
        Save generated assets into a directory, return item metadata.
        TODO: can an asset also be a full STAC item? In principle, one openEO job can either generate a full STAC collection, or one STAC item with multiple assets...
        ...
        return SaveResult.wrap_assets_in_item({"result.json":{
...
class AggregatePolygonResult(JSONResult):  # TODO: if it supports NetCDF and CSV, it's not a JSONResult
    ...
    def write_assets(self, directory: Union[str, Path]) -> Dict[str, StacAsset]:
        ...
        return SaveResult.wrap_assets_in_item({str(Path(filename).name): asset})
